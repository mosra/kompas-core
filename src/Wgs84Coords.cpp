...
double Wgs84Coords::distance(const Wgs84Coords& _a, const Wgs84Coords& _b) {
    double a = 6378137, b = 6356752.3142,  f = 1/298.257223563;  // WGS-84 ellipsoid params
    double L = (_a.longtitude()-_b.longtitude())*PI/180;
    double U1 = atan((1-f) * tan(_a.latitude()*PI/180));
    double U2 = atan((1-f) * tan(_b.latitude()*PI/180));
    ...
    /* FIXME is it safe not to declare default values? DO-WHILE should always run one or more times, or should all declaration be outside of loop? */
    ...
    do {
        ...
        /* FIXME Looks like checking if there was a division by zero, this happens when both _a and _b are on equatorial line*/
    ...
    /* FIXME What shall I return on fail? */
