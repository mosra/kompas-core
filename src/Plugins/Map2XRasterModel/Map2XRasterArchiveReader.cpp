...
Map2XRasterArchiveReader::Map2XRasterArchiveReader(const string& _file): _version(0), _total(0), _begin(0), _end(0), _isValid(false) {
    ...
    if(_version == 2) {
        ...
        file.read(buffer, 4);
        _total = Endianness::bigEndian(*reinterpret_cast<unsigned int*>(buffer));

        /* Beginning tile */
        file.read(buffer, 4);
        _begin = Endianness::bigEndian(*reinterpret_cast<unsigned int*>(buffer));

        /* (One item after) ending tile */
        file.read(buffer, 4);
        _end = Endianness::bigEndian(*reinterpret_cast<unsigned int*>(buffer));
...
string Map2XRasterArchiveReader::get(unsigned int tileNumber) {
    ...
    if(_version == 2) {
        ...
        file.seekg(16+4*(tileNumber-begin()));

        /* Position of tile data */
        file.read(buffer, 4);
        position = Endianness::bigEndian(*reinterpret_cast<unsigned int*>(buffer));

        /* Size of tile data */
        file.read(buffer, 4);
        size = Endianness::bigEndian(*reinterpret_cast<unsigned int*>(buffer))-position;
